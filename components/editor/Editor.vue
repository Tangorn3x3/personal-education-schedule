  ...
  <div class="prism-editor-wrapper">
    ...
    >
      ...
      <div
        ...
        :class="{ 'dense-code': dense }"
        v-for="(line, indx) in lineNumbersCount"
        :key="indx"
        @click.prevent="lineCopy(line)"
      >
        {{ indx + 1 }}
      </div>
    </div>
    <pre
      ...
      :class="{ ['language-' + lang]: true, 'dense-code': dense }"
      ref="pre"
      v-html="content"
      ...
      v-on="$listeners"
    ></pre>
    <div class="copy-btn" @click="copyAll">Copy</div>
  </div>
</template>
...
<script>
  ...
  import * as stringUtils from "@/utils/utils/stringUtils";
  ...
  export default {
    ...
    computed: {
      lang() {
        ...
        switch (result) {
          case 'sh': result = 'batch';break;
        }

        return result
      },
      content() {
        return prism(this.codeData || "", this.lang);
      },
      lineNumbersCount() {
        let lines = this.codeData.split(/\r\n|\n/)
    ...
    methods: {
      ...
      lineCopy (line) {
        ...
        stringUtils.copyToClipboardWithoutNavigator(result)
        console.log('Line copied')
      },
      copyAll() {
        stringUtils.copyToClipboardWithoutNavigator(this.codeData)
...
<style>
  ...
  .prism-editor__line-number:hover {
  ...
  .v-application code .title, code .title {
