<template>
  <v-card flat color="lightgrey">
    <v-img
      ...
      class="white--text align-end passport-img"
      height="200px"
    />
    ...
    <input
      ...
      accept="image/*"
    >
    <image-cropper-dialog v-if="showCropperDialog"
                          :image-src="selectedImagePic"
                          @crop="onImageCropped"
...
import { fileToImageBase64, imageBase64ToFile } from '@/utils/utils/fileUtils'
...
export default {
  ...
  methods: {
    ...
    async updateImage (e) {
      ...
      this.selectedImagePic = await fileToImageBase64(file)
      this.showCropperDialog = true
    },
    onImageCropped (imageBase64Data) {
      this.imagePic = imageBase64Data
      this.imageFile = imageBase64ToFile(imageBase64Data, this.selectedImageFile)
      this.$emit('change', this.base64 ? this.imagePic : this.imageFile)
...
<style scoped>
  .passport-img {cursor: pointer; transition: all 0.5s;}
  .passport-img:hover {filter: brightness(0.7);}
</style>
